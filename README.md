...
Owners manage their team through the SAME hardened invitation/membership RPCs the administrator uses (one shared database authorization gate; owner calls are additionally restricted to Manager/Staff targets, never themselves, never other owners, never accounts whose profile SnowBeltTech has deactivated (shown read-only as "Account deactivated"), with the exact-provenance invitation compensation unchanged). Owners can read only their own business's pending invitations; the access ledger stays administrator-only.
