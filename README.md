...
4. **Customer portal** — real Supabase Auth sign-in, customer dashboard with audited status/follow-up/notes/assignment, read-only admin overview and lead inspection (implemented locally); export and password reset pending.
5. **Platform administration** — business creation, suspend/reactivate, integration-source registration/activation, invitation-only customer provisioning and membership management with immutable operations and access ledgers (implemented locally); password reset/SMTP and MFA are later batches.
