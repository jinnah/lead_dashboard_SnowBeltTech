...
4. **Customer portal** — real Supabase Auth sign-in, customer dashboard with audited status/follow-up/notes/assignment, read-only admin overview and lead inspection (implemented locally); export, invitations and password reset pending.
5. **Platform administration** — business creation, suspend/reactivate, integration-source registration/activation with an immutable operations ledger (implemented locally); invitations and customer user provisioning are the next batch.
