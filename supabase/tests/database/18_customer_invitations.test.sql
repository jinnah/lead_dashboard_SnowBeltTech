...
select is((select count(*) from pg_policies where tablename in ('customer_invitations', 'customer_access_events')), 3::bigint, 'invitations carry the admin + owner SELECT policies; the ledger stays admin-only');
...
select throws_ok(format($$select * from public.admin_prepare_customer_invitation(%L, 'x@y.invalid', 'X', 'BUSINESS_OWNER')$$, :'biz_a'), '22023', null, 'business owner can never provision another OWNER');
select throws_ok(format($$select * from public.admin_set_business_member_role(%L, %L, 'BUSINESS_MANAGER')$$, :'biz_a', :'a_owner'), '22023', null, 'business owner can never change their OWN membership');
select throws_ok(format($$select * from public.admin_set_business_member_status(%L, %L, 'inactive')$$, :'biz_a', :'a_owner'), '22023', null, 'business owner can never deactivate themselves');
...
select is((select count(*) from public.customer_invitations), 0::bigint, 'no invitation rows exist for the owner to read yet');
